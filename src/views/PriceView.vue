...
import { exportPriceListToPDF } from '../utils/pdfExport'
...
const exportPDF = async () => {
  ...
  try {
    ...
    // 具体的字体内嵌、图片降采样、autoTable 排版与分页逻辑已抽离至 utils/pdfExport.js
    const count = await exportPriceListToPDF({
      data: allData,
      usdExchangeRate: usdExchangeRate.value,
      rmbExchangeRate: rmbExchangeRate.value,
      priceMode: pdfPriceMode.value,
      onProgress: (loaded, total) => { exportProgress.value = { loaded, total } },
